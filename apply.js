...
async function initializeApplyApp() {
  try {
    ...
    currentLineDisplayName = localStorage.getItem('lineDisplayName') || "LINEユーザー";
    ...
    if (result.status === "success") {
      ...
      if (approval === "承認済") {
        ...
        statusDisplay.innerHTML = `
          現在のステータス: <span class="text-success fw-bold">承認済 (${escapeHTML(role)})</span><br>
          氏名: ${escapeHTML(memberName)}<br>
          <span class="text-muted small mt-1 d-block">すでに承認されています。スケジュール画面へ移動します。</span>
        `;
        ...
        localStorage.setItem('lineUserId', lineUserId);
        statusDisplay.innerHTML = `
          現在のステータス: <span class="text-warning fw-bold">承認待ち（申請中）</span><br>
          申請された氏名: ${escapeHTML(memberName)}<br>
          <span class="text-muted small mt-1 d-block">管理者の承認をお待ちください。</span>
        `;
        regCard.classList.add("d-none");
      } else {
        localStorage.setItem('lineUserId', lineUserId);
        let statusHtml = `現在のステータス: <span class="text-secondary fw-bold">未登録</span>`;
        if (!isSystemAdminExists) {
          statusHtml += `<br><span class="text-danger fw-bold small mt-1 d-block">※システム管理者が未設定です。初期のシステム管理者登録を行います（キーワードが必要です）。</span>`;
        }
...
function setupFormDynamicFields() {
  ...
  if (roleSelect) roleSelect.addEventListener("change", updateFieldVisibility);
  if (appTypeSelect) appTypeSelect.addEventListener("change", updateFieldVisibility);
}
...
function updateFieldVisibility() {
  ...
  if (fieldAppType) fieldAppType.classList.add("d-none");
  if (fieldHouseholdName) fieldHouseholdName.classList.add("d-none");
  if (fieldHouseholdId) fieldHouseholdId.classList.add("d-none");
  if (fieldNote) fieldNote.classList.add("d-none");
  if (fieldKeyword) fieldKeyword.classList.add("d-none");

  if (role === CONFIG.ROLES.SYSTEM_ADMIN) {
    if (fieldKeyword) fieldKeyword.classList.remove("d-none");
  } else if (role === CONFIG.ROLES.HOUSEHOLD_ADMIN) {
    if (fieldAppType) fieldAppType.classList.remove("d-none");
    if (fieldNote) fieldNote.classList.remove("d-none");

    const appTypeSelect = document.getElementById("applicationType");
    const appType = appTypeSelect ? appTypeSelect.value : "";

    if (appType === "新規登録") {
      if (fieldHouseholdId) fieldHouseholdId.classList.remove("d-none");
      if (fieldHouseholdName) fieldHouseholdName.classList.remove("d-none");
...
async function submitApplication() {
  ...
  try {
    ...
    if (result.status === "success") {
      localStorage.setItem('lineUserId', currentLineUserId);
      localStorage.setItem('lineDisplayName', currentLineDisplayName);
...
function showAppMessage(message, type = "error") {
  ...
  msgBox.innerHTML = `<span class="block">${escapeHTML(message)}</span>`;
  msgBox.classList.remove("d-none");

  if (type === "success") {
    msgBox.className = "mb-3 p-3 rounded small alert alert-success";
  } else {
    msgBox.className = "mb-3 p-3 rounded small alert alert-danger";
  }
}
